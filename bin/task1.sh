...
if [ "$WITH_PREPROCESS" == "True" ]; then
	...
	cat data/train-task-1-history.tsv data/dev-task-1-history.tsv data/test-task-1-history.tsv | python build_vocabulary.py > data/vocab.tsv
fi

python train.py --train data/train-task-1-history.tsv --dev data/dev-task1-history-500.tsv